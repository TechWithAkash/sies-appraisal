...
export default function FileUpload({
  ...
  const handleFileSelect = (file) => {
    ...
    const fileData = {
      ...
      // In real app, this would be uploaded to server and return a URL
      ...
      {!value ? (
        ...
        >
          ...
          <div className={`mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full ${
            dragOver ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-200 text-slate-500'
          }`}>
            <Upload size={24} />
          </div>
        ...
        <div className={`rounded-xl border-2 ${displayError ? 'border-red-200 bg-red-50' : 'border-emerald-200 bg-emerald-50'} p-4`}>
          <div className="flex items-center gap-4">
            <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${
              displayError ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'
            }`}>
              <FileIcon size={24} />
            </div>
            ...
            <div className="flex items-center gap-1">
              {showPreview && value.url && (
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); window.open(value.url, '_blank'); }}
